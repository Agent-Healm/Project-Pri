...
public class Shotgun : Weapon
{
    ...
    [SerializeField] private int m_shotgunCount = 1;
    public int GetShotgunCount { get => m_shotgunCount; }
    [SerializeField] private Shotgun.Category m_shotgunType = Shotgun.Category.Wide;
    public Shotgun.Category GetShotgunType { get => m_shotgunType; }

    // private int m_shotgunCount;
    // private Category m_shotgunType;
    ...
    public override void Action(Vector2 l_direction, Vector2 l_position){
        if (m_shotgunType == Category.Wide){
            float l_shotgunSpread = m_deviation * 2f / (m_shotgunCount - 1);
            float l_deg = Vector2.SignedAngle(Vector2.right, l_direction);

            if (m_shotgunCount % 2 == 1){
                this.Attack(l_deg, l_position);
            }

            for (int i = 1 + m_shotgunCount % 2; i <= m_shotgunCount; i+=2){
                this.Attack(l_deg + l_shotgunSpread * i * 0.5f, l_position);
                this.Attack(l_deg - l_shotgunSpread * i * 0.5f, l_position);
            }

        } 
        else if (m_shotgunType == Category.Scatter){
            float deg = Vector2.SignedAngle(Vector2.right, l_direction);

            for (int i = 0; i < m_shotgunCount; i++){
